    ...
    IconPicker.prototype.render = function (options) {
        ...
        var a = {
            ...
            createSelect: function () {
                var oriIcon = '<i class="ri-">';

                // 默认图标
                if (ORIGINAL_ELEM_VALUE === '') {
                    if (isFontClass) {
                        ORIGINAL_ELEM_VALUE = 'ri-home-8-line';
                    } else {
                        ORIGINAL_ELEM_VALUE = 'amp;#xee25;';
                    }
                }

                if (isFontClass) {
                    oriIcon = '<i class="ri- ' + ORIGINAL_ELEM_VALUE + '">';
                } else {
            ...
            createList: function (text) {
                ...
                for (var i = 0; i < l; i++) {
                    ...
                    if (isFontClass) {
                        icon += '<i class="ri- ' + obj + '"></i>';
                    } else {
                        icon += '<i class="ri-">' + obj.replace('amp;', '') + '</i>';
                    }
                    icon += '</div>';
            ...
            check: function () {
                ...
                a.event('click', item, function (e) {
                    var el = $(e.currentTarget).find('.ri-'),
                        icon = '';
                    if (isFontClass) {
                        var clsArr = el.attr('class').split(/[\s\n]/),
                            cls = clsArr[1],
                            icon = cls;
                        $('#' + TITLE_ID).find('.layui-iconpicker-item .ri-').html('').attr('class', clsArr.join(' '));
                    } else {
                        var cls = el.html(),
                            icon = cls;
                        $('#' + TITLE_ID).find('.layui-iconpicker-item .ri-').html(icon);
                    }
